...
class DorisHelper:
    _pool = None
    _hosts = []
    _current_host_index = 0

    @classmethod
    async def init_pool(cls, hosts, port=9030, user='root', password='',
                       database='', min_size=1, max_size=5, retries=3):
        """支持多节点初始化的连接池"""
        ...
        for attempt in range(retries):
            ...
            try:
                ...
                logger.info(f"Connected to Doris host: {host}")
                return
            except Exception as e:
                logger.warning(f"Failed to connect to {host}, attempt {attempt + 1}/{retries}: {str(e)}")
                if attempt == retries - 1:
                    raise Exception(f"All connection attempts failed. Last error: {str(e)}")

    @classmethod
    def _get_next_host(cls):
        """轮询获取下一个可用主机"""
        host = cls._hosts[cls._current_host_index % len(cls._hosts)]
        cls._current_host_index += 1
        return host
    ...
    async def close(cls):
        ...
        if cls._pool is not None:
            ...
            cls._pool = None
            cls._instance = None
            logger.info("Doris connection pool closed.")
